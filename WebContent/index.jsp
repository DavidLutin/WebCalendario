<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%@ page import="java.sql.*" %>
     ...
     <% 
     	...
     	if(session.getAttribute("anio") == null){session.setAttribute("anio",2015);}
...
<html>
	...
	<td id="aniotitulo">
		...
		try{
			int desfase = 0;
			if(session.getAttribute("mes").toString().equals("1")){desfase = 3;}
			if(session.getAttribute("mes").toString().equals("2")){desfase = 6;}
			if(session.getAttribute("mes").toString().equals("3")){desfase = 6;}
			if(session.getAttribute("mes").toString().equals("4")){desfase = 2;}
			if(session.getAttribute("mes").toString().equals("5")){desfase = 4;}
			if(session.getAttribute("mes").toString().equals("6")){desfase = 0;}
			if(session.getAttribute("mes").toString().equals("7")){desfase = 2;}
			if(session.getAttribute("mes").toString().equals("8")){desfase = 5;}
			if(session.getAttribute("mes").toString().equals("9")){desfase = 1;}
			if(session.getAttribute("mes").toString().equals("10")){desfase = 3;}
			if(session.getAttribute("mes").toString().equals("11")){desfase = 6;}
			if(session.getAttribute("mes").toString().equals("12")){desfase = 1;}
			...
			for(int dia = 1;dia <= 31;dia++){
				String peticion = "SELECT idevento,eventos.nombre AS nombre,anio,mes,dia,hora,minuto,segundo,duracion,color FROM eventos LEFT JOIN calendarios ON eventos.calendario = calendarios.idcalendario WHERE anio = 2015 AND mes = "+session.getAttribute("mes")+" AND dia = "+dia+" AND usuario = "+session.getAttribute("idusuario")+"";
				rset = stmt.executeQuery(peticion);
				out.println("<div class='dia ui-droppable' dia='"+dia+"' style='position:relative;''><div class='numerodia'><b>"+dia+"</b></div>");
				while(rset.next()){
					out.println("<div idevento = '"+rset.getString("idevento")+"' idcaldia='7' anio='2015' mes='9' dia='13' hora='9' nombre='IMF' class='evento ui-draggable' style='background:rgb("+rset.getString("color")+");width:90%;height:"+(rset.getInt("duracion")*0.1)+"%;position:absolute;top:"+((rset.getInt("hora")*4)+12)+"%;'>"+rset.getInt("hora")+"<span class='motivoevento'>-"+rset.getString("nombre")+"</span><div class='eliminar'><a style='color:#d7d7d7;' href='acciones/eliminarEvento.jsp?idevento="+rset.getInt("idevento")+"'>X</a></div></div></div>");
				}
				out.println("</div>");
		...
		%>
			...
			<div id="nuevoevento" style="display: none;">
				<div id="contieneform">
					...
					<form action="http://localhost:8084/WebCalendario/acciones/nuevoEvento.jsp" method="POST">
					<input type="hidden" name="anio" value="2015">
		...
		<div id="nuevocalendarioform" style="display: none;">
				<div id="contieneform">
					...
					<form action="http://localhost:8084/WebCalendario/acciones/nuevoCalendario.jsp" method="POST">
					<input type="hidden" name="anio" value="2015">
						<input type="hidden" name="mes" value="9">
